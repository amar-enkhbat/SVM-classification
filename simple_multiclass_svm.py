...
X_train_std = np.copy(X.astype(float))     
X_train_std[:, 0] = (X_train[:, 0] - mean(X_train[:, 0])) / standard_deviation(X_train[:, 0])
X_train_std[:, 1] = (X_train[:, 1] - mean(X_train[:, 1])) / standard_deviation(X_train[:, 1])
...
# =============================================================================
number_of_classifiers = number_of_classes * (number_of_classes - 1.0) / 2.0
...
# =============================================================================
rgen = np.random.RandomState(random_state)
weight = rgen.normal(loc = 0.0, scale = 0.1, size = (X_train_std.shape[1] + 1, number_of_classes))

# =============================================================================
# Plot
# =============================================================================
def plot_decision_boundary(weight, label):
    t = np.arange(-2, 2, 0.1)
    plt.plot(t, -(weight[0] + weight[1] * t)/weight[2], label = label)
    #plt.savefig('graph1.png')
    
for i in range(len(weight)):
    plot_decision_boundary(weight[i], str(i))
...
for i in range(number_of_classes):
    for j in range(i + 1, number_of_classes):
        ...
        mapped_y = class_mapping(mapped_y)#X_train_std = (X_train - mean(X_train)) / standard_deviation(X_train)
...
#plt.legend()
        ...
        for k in range(len(weight)):
            plot_decision_boundary(weight[k], str(k))
        print(i, j)    
        plt.scatter(X_train_std[y_train == 1, 0], X_train_std[y_train == 1, 1], label = "Class 0", marker = "x")
        plt.scatter(X_train_std[y_train == 2, 0], X_train_std[y_train == 2, 1], label = "Class 1", marker = "v")
        plt.scatter(X_train_std[y_train == 3, 0], X_train_std[y_train == 3, 1], label = "Class 2", marker = "*")
